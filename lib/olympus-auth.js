...
function getExpectedToken() {
  return process.env.OLYMPUS_TOKEN || process.env.NEXT_PUBLIC_OLYMPUS_TOKEN || "olympus2026";
}
...
export async function requireAuthJWT(request) {
  ...
  const auth = request.headers.get("authorization");
  const expectedToken = process.env.OLYMPUS_TOKEN || process.env.NEXT_PUBLIC_OLYMPUS_TOKEN || "olympus2026";
